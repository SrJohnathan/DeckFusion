...
class Card2dEditor(var card: Cards?) : Table() {


    val ataque = Label("2000", TextLouder.styleBebasBorder(Color.WHITE, 18))
    val defeza = Label("0000", TextLouder.styleBebasBorder(Color.WHITE, 18))
    var stars = Table()
    val attr = Image()
    val img1 = Image()
    val tableNum = Table()
    ...
    fun init(cardMy: Cards) {
        ...
        for (i in 1..cardMy.star) {
            ...
            stars.add(star).width(15f).height(15f)
        }
        ...
        stars2.add(attr).width(20f).height(20f).right().padRight(10f)


        this.background = TextureRegionDrawable(LouderFull.card2dBig?.findRegion(cardMy.cardcolor.toString()))

        this.add(img1)?.top()!!.expandX().fillX().padRight(1f).padLeft(1f).height(180f).width(170f).padTop(5f)
        this.row()
        ...
        tableNum.add(ataque)!!.expandX().left().padLeft(20f)
        tableNum.add(defeza)!!.expandX().right().padRight(20f)
